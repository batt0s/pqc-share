...
def verify_host(
    target_host: str, public_key_bytes: bytes, key_dir="~/.pqc_share"
) -> bool:
    ...
    with open(known_pairs_path, "r") as f:
        for line in f:
            ...
            if line and not line.startswith("#"):
                host, fpr = line.split(" ", 1)
                known_hosts[host] = fpr

    if target_host in known_hosts:
        if known_hosts[target_host] == fingerprint:
            ...
            print(f"Expected fingerprint : SHA256:{known_hosts[target_host]}")
        ...
        print(f"[*] McEliece348864 key fingerprint is SHA256:{fingerprint}.")
        while True:
            ans = (
            ...
            if ans in ["yes", "y"]:
                with open(known_pairs_path, "a") as f:
                    f.write(f"{target_host} {fingerprint}\n")
                print(
                    f"[+] Warning: Permanently added '{target_host}' to the list of known pairs."
                )
                return True
            elif ans in ["no", "n"]:
                return False
